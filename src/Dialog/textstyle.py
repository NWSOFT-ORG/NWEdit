...
class StyleWindow(WinFrame):
    def __init__(self, master, text, handler):
        ...
        self.font = ttk.Combobox(
            self, state="readonly", values=listfonts(), textvariable=self.font_var
        )
    ...
    def update_text(self, *_):
        ...
        ascii_text = ""
        try:
            ascii_text = art.text2art(
                self.entry.get(), font=self.font.get(), chr_ignore=False
            )
        except art.artError:
            ascii_text = art.text2art(
                self.entry.get(), font=self.font.get(), chr_ignore=True
            )
        finally:
