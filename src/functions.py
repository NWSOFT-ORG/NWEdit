...
def is_dark_color(hex_code):
	if hex_code.startswith('#'):
		hex_code = hex_code[1:]
	if hex2dec(hex_code[:2]) <= DARK_COLOR and hex2dec(
	    hex_code[2:4]) <= DARK_COLOR and hex2dec(hex_code[4:]) <= DARK_COLOR:
		return True
	return False
...
def _run_in_terminal_in_macos(cmd, cwd, env_overrides, keep_open):
	...
	cmds = ("export PYPLUSPREFIX=" + common_prefix + " ; " +
	...
	do_script_cmd2 = ("""        do script %s in window 1 """ %
